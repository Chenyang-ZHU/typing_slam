...
Visualizer::Visualizer() {
  viewer_thread_ = std::thread(std::bind(&Visualizer::ThreadLoop, this));
}
...
void Visualizer::AddCurrentFrame(Frame::Ptr current_frame) {
...
cv::Mat Visualizer::PlotFrameImage() {
  cv::Mat img_out;
  cv::cvtColor(current_frame_->image_, img_out, cv::COLOR_GRAY2BGR);
...
void Visualizer::DrawFrame(Frame::Ptr frame, const float* color) {
  ...
  Sophus::Matrix4d m = Twc.matrix().template cast<double>();
